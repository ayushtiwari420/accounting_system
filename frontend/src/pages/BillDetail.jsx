...
const BillDetail = () => {
  ...
  const fetchBill = async () => {
    try {
      ...
      const paid = Number(billData.paid_amount || 0);
      ...
      setError(err.response?.data?.message || "Vendor bill not found");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRegisterPayment = async (e) => {
    ...
    try {
      ...
      console.error("Error registering vendor payment:", err);
      setError(err.response?.data?.message || "Failed to process vendor payment");
    } finally {
      setActionLoading(false);
    }
  };

  if (loading) return <Loading text="Loading vendor bill document & AP GL trace..." />;

  if (error && !bill) {
    return (
      <div className="space-y-4">
        <Link to="/bills" className="text-xs text-blue-600 hover:underline">
          &larr; Back to Bills
        </Link>
        <div className="bg-rose-50 border border-rose-200 text-rose-700 p-6 rounded-xl text-center">
          <h2 className="text-lg font-bold mb-1">Vendor Bill Error</h2>
  ...
  const items = bill.vendor_bill_items || [];
  const vendor = bill.contacts || {};
  const totalAmount = Number(bill.total_amount || 0);
  const paidAmount = Number(bill.paid_amount || 0);
  const remainingDue = Math.max(0, totalAmount - paidAmount);
  const isFullyPaid = bill.status === "PAID" || remainingDue <= 0.01;

  // Workflow timeline steps
  const steps = [
    { key: "POSTED", label: "Bill Received & Posted" },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <PageHeader
        ...
        subtitle={`Vendor Bill for ${vendor.name || "Supplier"}`}
        breadcrumbs={[
          ...
          <div className="flex items-center space-x-3">
            ...
            {!isFullyPaid && (
              ...
              >
                ...
                <span>Register Vendor Disbursement</span>
              </button>
            )}
          </div>
        }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest block">Total Billed</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest block">Total Paid Out</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-1">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest block">Remaining Balance</span>
      ...
      {/* Printable Vendor Bill Details */}
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        <div className="flex justify-between items-start border-b border-slate-200 pb-6">
          <div>
            <span className="text-xs font-black text-blue-900 uppercase tracking-widest block">VENDOR BILL</span>
            <h2 className="text-2xl font-black text-blue-950">{vendor.name}</h2>
            <p className="text-xs text-slate-500 font-medium">{vendor.address || "Supplier Address"}</p>
          </div>
          <div className="text-right">
            <h3 className="text-xl font-bold font-mono text-slate-800">{bill.bill_number}</h3>
                ...
                return (
                  <tr key={item.id || idx} className="hover:bg-blue-50/30">
                    <td className="py-3.5 px-3 text-slate-900 font-semibold">
                      {product.name || item.description || "Vendor Item"}
                    </td>
      ...
      <AccountingTrace journalEntry={bill.journal_entries} title="Automatic Accounts Payable GL Traceability" />

      {/* Payment Registration Modal */}
      <Modal
        isOpen={showPaymentModal}
        onClose={() => setShowPaymentModal(false)}
        title={`Register Vendor Payment for ${bill.bill_number}`}
      >
        <form onSubmit={handleRegisterPayment} className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-slate-100">
            ...
            >
              {actionLoading ? "Processing Payment..." : "Record Vendor Payment"}
            </button>
          </div>
        </form>
      </Modal>
